...
Shader::Shader(ID3D12Device* device,
	...
	bool depthEnable, CullConfig cullMode)
	: m_enabled(false)
{
	...
	if (!createGPipelineState(device, vsBlob.Get(), psBlob.Get(),
		renderFormat, inputLayouts, layoutNum, blend, depthEnable,
		cullMode)) return;
...
bool Shader::createGPipelineState(ID3D12Device* device, ID3DBlob* vertexShader,
	...
	D3D12_INPUT_ELEMENT_DESC* inputLayouts, UINT layoutNum,
	BlendConfig blend, bool depthEnable, CullConfig cullMode)
{
	...
	pDesc.RasterizerState.CullMode = (cullMode == CullConfig::None) ?
		D3D12_CULL_MODE_NONE : ((cullMode == CullConfig::Back) ?
			D3D12_CULL_MODE_BACK : D3D12_CULL_MODE_FRONT);
