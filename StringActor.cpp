...
StringActor::StringActor(Scene* scene, const wchar_t* str, FontData& fontData,
	const XMFLOAT2& fontSize, const XMFLOAT2& pos, const XMFLOAT2& vel,
	float angle, float angleVel, const XMFLOAT2& scale,
	float tracking, int maxStringLength, int shaderIndex)
	...
	, m_indices(fontData.indices)
{
	...
	for (size_t i = 0; i < m_string.size(); ++i)
	{
		...
		BlockSpriteActor* actor = new BlockSpriteActor(m_scene,
			...
			m_scale, m_angle, 0.0f, &m_fontSize, false, fontData.ddsFlag);
		if (!actor->isEnabled())
		{
			delete actor;
			releaseActors();
