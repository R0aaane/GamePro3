...
float4 NormalMapPS(VS_Output input) : SV_TARGET
{
    // ノーマルマッピング
    float3 nvec = nmap.Sample(smp, input.uv).xyz;
    nvec = nvec * 2.0f - 1.0f;
    
    float3 n = normalize(nvec.x * input.tangentW +
        nvec.y * input.binormalW + nvec.z * input.normW);
