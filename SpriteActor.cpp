...
SpriteActor::SpriteActor(Scene* scene, const std::wstring& filePath,
	...
	XMFLOAT2* spriteSize, XMFLOAT2* uvSize,	bool centerFlag, bool ddsFlag)
	...
	, m_alpha(1.0f)
{
	...
	SpriteTransData d;
	m_modelIndex = renderer->allocateConstBuffer(&d, sizeof(SpriteTransData), false);
	if (m_modelIndex == -1) return;
	m_imgData = renderer->allocateShaderResource(
		filePath, ddsFlag);
...
void SpriteActor::draw()
{
	...
	XMFLOAT2 scale = 
		XMFLOAT2(m_scale.x * m_spriteSize.x, m_scale.y * m_spriteSize.y);
